...
class SECFetcher:
    ...
    def fetch_latest_10k(self, ticker: str) -> str:
        ...
        try:
            ...
            raise SECFetchError(
                f"Failed to fetch 10-K for {ticker}", details={"ticker": ticker, "error": str(e)}
            )
    ...
    def _get_cik(self, ticker: str) -> str:
        ...
        try:
            ...
            if not company_info:
                ...
                if not entries:
                    raise InvalidTickerError(
                        f"Ticker '{ticker}' not found in SEC database", details={"ticker": ticker}
                    )
            ...
            raise InvalidTickerError(
                f"Could not extract CIK for ticker '{ticker}'", details={"ticker": ticker}
            )
    ...
    def _find_latest_filing_url(self, cik: str) -> str:
        ...
        try:
            ...
            if not entry:
                raise SECFetchError(f"No 10-K filing found for CIK {cik}", details={"cik": cik})
            ...
            raise SECFetchError(f"Could not find filing URL for CIK {cik}", details={"cik": cik})

        except requests.RequestException as e:
            logger.error(f"Network error while finding filing for CIK {cik}: {e}")
            raise SECFetchError(
                f"Failed to find 10-K filing for CIK {cik}", details={"cik": cik, "error": str(e)}
            )
    ...
    def _extract_document_url(self, index_url: str) -> str:
        ...
        try:
            ...
            raise SECFetchError(
                "Could not extract document URL from index page", details={"index_url": index_url}
            )

        except requests.RequestException as e:
            logger.error(f"Error extracting document URL: {e}")
            raise SECFetchError(
                "Failed to extract document URL", details={"index_url": index_url, "error": str(e)}
            )
    ...
    def _extract_document_url_from_detail(self, detail_url: str) -> str:
        ...
        try:
            ...
            raise SECFetchError(
                "Could not find documents link", details={"detail_url": detail_url}
            )

        except requests.RequestException as e:
            logger.error(f"Error fetching detail page: {e}")
            raise SECFetchError(
                "Failed to fetch detail page", details={"detail_url": detail_url, "error": str(e)}
            )
    ...
    def _fetch_filing_html(self, url: str) -> str:
        ...
        try:
            ...
            if "risk factors" not in html_lower and "item 1a" not in html_lower:
                logger.warning(
                    "Fetched HTML may not be a valid 10-K (no risk factors section found)"
                )
            ...
            raise SECFetchError(
                "Failed to fetch filing HTML", details={"url": url, "error": str(e)}
            )
