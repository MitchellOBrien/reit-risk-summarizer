...
class GoldenRecord:
    ...
    def risks(self) -> list[str]:
        ...
        return [self.risk_1, self.risk_2, self.risk_3, self.risk_4, self.risk_5]
...
class Evaluator:
    ...
    def evaluate_single(self, ticker: str, generated_risks: list[str]) -> EvaluationResult:
    ...
    def evaluate_batch(self, results: dict[str, list[str]]) -> list[EvaluationResult]:
        ...
        """
        return [self.evaluate_single(ticker, risks) for ticker, risks in results.items()]

    def get_aggregate_metrics(self, results: list[EvaluationResult]) -> dict[str, float]:
        ...
        return {
            ...
            "mean_sector_specificity": sum(r.sector_specificity for r in results) / len(results),
            "num_evaluated": len(results),
        }

    def get_sector_breakdown(self, results: list[EvaluationResult]) -> dict[str, dict[str, float]]:
    ...
    def get_golden_record(self, ticker: str) -> GoldenRecord:
        ...
        """
        if ticker not in self.golden_records:
            raise ValueError(f"Ticker '{ticker}' not found in golden dataset")
